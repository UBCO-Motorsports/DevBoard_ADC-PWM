                         ...
                         const ftm_pwm_param_t * param)
{
    ...
    if ((NULL != state) && (FTM_MODE_NOT_INITIALIZED == state->ftmMode))
    {
        ...
        for (index = 0U; index < param->nNumIndependentPwmChannels; index++)
        {
            ...
            FTM_DRV_SetChnEdgeLevel(ftmBase, channelId, (uint8_t)param->pwmIndependentChannelConfig[index].levelSelect);
            ...
            {
                FTM_DRV_SetChnOutputPolarityCmd(ftmBase, channelId, ((param->pwmIndependentChannelConfig[index].polarity == FTM_POLARITY_HIGH)? true : false));
            }
        ...
        for (index = 0U; index < param->nNumCombinedPwmChannels; index++)
        {
            ...
            FTM_DRV_SetDualEdgeCaptureCmd(ftmBase, chnlPairNum, false);
            /* Configure the ELSB bit and ELSA bit from user */
            FTM_DRV_SetChnEdgeLevel(ftmBase, channelId, (uint8_t)param->pwmCombinedChannelConfig[index].levelSelect);
            /* Set MSB and MSA bits */
            FTM_DRV_SetChnMSnBAMode(ftmBase, channelId, 3U);
            ...
            if (param->pwmCombinedChannelConfig[index].enableSecondChannelOutput)
            {
                ...
                /* Configure the ELSB bit and ELSA bit from user */
                FTM_DRV_SetChnEdgeLevel(ftmBase, (uint8_t)(channelId + 1U), (uint8_t)param->pwmCombinedChannelConfig[index].levelSelectOnNextChn);
                /* Configure complementary mode for channel (n+1) */
                FTM_DRV_SetDualChnCompCmd(ftmBase, chnlPairNum, ((param->pwmCombinedChannelConfig[index].secondChannelPolarity == FTM_MAIN_INVERTED)? true : false));
            }
            else
            {
                /* Set polarity for the main channel */
                FTM_DRV_SetChnOutputPolarityCmd(ftmBase, channelId, ((param->pwmCombinedChannelConfig[index].mainChannelPolarity == FTM_POLARITY_HIGH)? true : false));
            ...
            FTM_DRV_SetChnTriggerCmd(ftmBase, (chnlPairNum << 1U), param->pwmCombinedChannelConfig[index].enableExternalTrigger);
            /* Enable the generation a trigger on the channel (n+1) */
            FTM_DRV_SetChnTriggerCmd(ftmBase, (chnlPairNum << 1U) + 1U, param->pwmCombinedChannelConfig[index].enableExternalTriggerOnNextChn);
        }
        ...
        for (index = 0U; index < param->nNumIndependentPwmChannels; index++)
        {
            ...
            if ((param->pwmIndependentChannelConfig[index].enableSecondChannelOutput) && (param->pwmIndependentChannelConfig[index].secondChannelPolarity == FTM_MAIN_DUPLICATED))
            {
                /* Write CnV registers and setup duty cycle and phase values */
                retVal = FTM_DRV_UpdatePwmChannel(instance,
                                                  hwChannel + 1U,
